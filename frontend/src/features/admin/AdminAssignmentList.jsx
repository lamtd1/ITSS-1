...
const AdminAssignmentList = () => {
  ...
  const fetchAssignments = async () => {
    try {
      ...
      const response = await axios.get(`${baseBackendURL}/assignments?userId=${CURRENT_TEACHER_ID}`);
  ...
  if (loading) {
      return <div className="p-8 text-center text-gray-500">読み込み中... (Đang tải...)</div>;
  }
  ...
  if (error) {
      return (
        <div className="p-8 text-center text-red-500">
            <p>{error}</p>
            <button onClick={fetchAssignments} className="mt-4 underline">再試行 (Thử lại)</button>
        </div>
      );
  }
          ...
          <tbody className="divide-y divide-gray-100">
            {assignments.length > 0 ? (
                assignments.map((asm) => {
                    ...
                    const count = (asm.assignedStudents && asm.assignedStudents.length) 
                                  || asm.assigneesCount 
                                  || 0;
                    ...
                    return (
                      <tr key={asm.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4">
                          ...
                          <div className="text-xs text-gray-500">作成日: {formatDate(asm.createdAt || asm.createdDate)}</div>
                        </td>
